...
- `apps/kimi-desktop-tauri`: the Tauri 2 + Svelte 5 desktop client — this repo's desktop focus (the former Electron app and the `apps/kimi-web` frontend were removed; unlike upstream, this fork does not ship the prebuilt `dist-web` browser bundle either — the daemon boots API-only for the embedded Tauri client). A fresh Svelte frontend talking to the kap-server daemon over REST + WS directly from the WebView; it must not depend on `@moonshot-ai/agent-core` (wire types are copied locally), and Tauri IPC is reserved for native-only ops. Requires the Rust toolchain for `tauri:dev` / `tauri:build`; its `typecheck` (svelte-check) is not wired into the root `pnpm run typecheck` — run `pnpm --filter @moonshot-ai/kimi-desktop-tauri run typecheck` explicitly. See `apps/kimi-desktop-tauri/AGENTS.md`.
