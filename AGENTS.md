...
- `packages/protocol`: shared REST + WebSocket protocol schemas (envelope, error codes, pagination, ws-control) for the daemon/server.
- `packages/acp-adapter`: Agent Client Protocol (ACP) adapter exposing kimi-code agents over ACP.
- `packages/pi-tui`: vendored fork of the pi-tui terminal UI rendering library. See `packages/pi-tui/AGENTS.md` — it has hardcoded local divergences from upstream that must be preserved on every re-vendor.
- `packages/oauth`: Kimi OAuth and managed auth utilities.
- `packages/migration-legacy`: migrates legacy `kimi-cli` data (`~/.kimi/`) into kimi-code (`~/.kimi-code/`).
...
- `docs/`: VitePress documentation site with mirrored `docs/en/` and `docs/zh/` locales. Edit both locales in the same change. See `docs/AGENTS.md`.
...
Run from the repo root unless noted. Per-package: add `-C <path>` or `--filter <pkg>` (e.g. `--filter @moonshot-ai/agent-core`).

- Build (all): `pnpm run build` · packages only: `pnpm run build:packages`
- Typecheck (all, builds packages first): `pnpm run typecheck`
- Lint (type-aware oxlint): `pnpm run lint` · autofix: `pnpm run lint:fix`
- Dependency-check (monorepo): `pnpm run sherif`
- Test (all, vitest run): `pnpm run test` · watch: `pnpm run test:watch` · coverage: `pnpm run test:coverage`
- Focused test file / pattern: `pnpm vitest run <path-or-pattern>` (or `pnpm -C <pkg> vitest run <pattern>`); note many packages run `build:packages` before their tests.
